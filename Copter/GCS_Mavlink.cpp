...
GCS_MAVLINK_Copter::data_stream_send(void)
{
    ...
    if (stream_trigger(STREAM_EXTRA1)) {                    //----------------------unsafe change-----------------------------
    ...
    if (stream_trigger(STREAM_EXTRA3)) {                    //----------------------unsafe change-----------------------------
        /*
...
void GCS_MAVLINK_Copter::handleMessage(mavlink_message_t* msg)
{
    ...
    case MAVLINK_MSG_ID_SET_VELOCITY_BODY://change---------------------------------
    {
        // decode packet
        //copter.gcs_send_text(MAV_SEVERITY_WARNING,"new message");
        mavlink_set_velocity_body_t packet;
        mavlink_msg_set_velocity_body_decode(msg,&packet);
        // exit if vehicle is not in Guided mode or Auto-Guided mode
        if ((copter.control_mode != GUIDED) && (copter.control_mode != GUIDED_NOGPS) && !(copter.control_mode == AUTO && copter.auto_mode == Auto_NavGuided))
        {
            break;
        ...
        // convert thrust to climb rate
        /*packet.vz+=0.5;
        packet.vz = constrain_float(packet.vz, 0.0f, 1.0f);
        float climb_rate_cms = 0.0f;
        if (is_equal(packet.vz, 0.5f)) {
            climb_rate_cms = 0.0f;
        } else if (packet.vz > 0.5f) {
            // climb at up to WPNAV_SPEED_UP
            climb_rate_cms = (packet.vz - 0.5f) * 2.0f * copter.wp_nav->get_speed_up();
        } else {
            // descend at up to WPNAV_SPEED_DN
            climb_rate_cms = (0.5f - packet.vz) * 2.0f * -fabsf(copter.wp_nav->get_speed_down());
        }*/
        Vector3f velocity3;
        velocity3=Vector3f(packet.vx,packet.vy,packet.vz);
        copter.guided_set_new_velocity(velocity3);
        break;
    }
    ...
    case MAVLINK_MSG_ID_SET_PID:        // MAV ID: 94
    {
        mavlink_set_pid_t packet;
        mavlink_msg_set_pid_decode(msg,&packet);
        copter.set_pid(packet.controller_id,packet.kP,packet.kI,packet.kD,packet.imax);
        break;
    }
    case MAVLINK_MSG_ID_SET_YAW:       // MAV ID: 95
    {
        mavlink_set_yaw_t packet;
        mavlink_msg_set_yaw_decode(msg,&packet);
        copter.set_yaw(packet.Yaw);
        break;
    }
    case MAVLINK_MSG_ID_NEW_LAND:
    {
        mavlink_new_land_t packet;
        mavlink_msg_new_land_decode(msg,&packet);
        Vector3f velocity3;
        velocity3=Vector3f(packet.vx,packet.vy,packet.vz);
        copter.guided_new_landing(velocity3,packet.disarm_height,packet.climb_rate,packet.throttle);
        break;
    }
