...
public:
    ...
    friend class Sc_RPY_Queue;
    friend class Sc_altitude_filter;
    friend class get_rangefinder;
    Copter(void);
...
#ifdef ScModifies
    void guided_new_vel_control_start();
#endif
...
#ifdef ScModifies
    //void guided_set_new_velocity(const Vector3f& velocity, float _kp, float _ki, float _kd, float _climb_rate_cms);
    void guided_set_new_velocity(const Vector3f& velocity);
    void guided_new_landing(const Vector3f& velocity, float _desired_disarm_height, float _desired_climbrate, float _desired_throttle);
    void set_pid(int controller_id, float _kp, float _ki, float _kd, float _Imax);
...
#ifdef ScModifies
     void guided_new_vel_control_run();
#endif
