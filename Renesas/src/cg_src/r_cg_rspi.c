...
extern volatile int spi_rx_idle;
extern volatile int spi_tx_idle;
/* End user code. Do not edit comment generated here */
...
static const uint32_t spi_timeout = 20;
uint32_t senddata = 0x40;
...
uint8_t SPI_Recv_One(void)
{
	...
	while(spi_rx_idle == 0 && currenttime() - entrytime < spi_timeout)
		R_RSPI0_Send_Receive(&senddata,1,recvtmp);

	PORT3.PODR.BIT.B0 = 1U;

	return_8 = *recvtmp;

	if(currenttime() - entrytime < spi_timeout)
		return(return_8);
	else
		return 0xFF;
}

void SPI_Change_Send_Flag(uint32_t send)
{
	senddata = send;
}
/* End user code. Do not edit comment generated here */
